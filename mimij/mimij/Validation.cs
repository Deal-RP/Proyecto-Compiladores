...
namespace mimij
{
    public class Validation
    {
        ...
        static public int identificar(string lexema)
        {
            ...
            {
                //PALABRA RESERVADA
                "^void$|^interface$|^double$|^boolean$|^string$|^class$|^static$|^int$|^null$|^this$|^extends$|^implements$|^for$|^while$|^if$|^else$|^return$|^break$|^New$|^System.out.println$|^System$",
                //BOOLEAN
                "^true$|^false$",
                //ENTERO
                "^[0-9]+$",
        ...
        public static void fileWriting(string name, int line, int columnFirst, int tipo)
        {
            ...
            else
            {
                ...
                ASemantico.TokensList.Add(new Token(name, line, columnFirst, tipo, string.Empty, string.Empty, string.Empty));
            }
